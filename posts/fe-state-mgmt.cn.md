...
- **性能**：**如果库的性能不能进入第一梯队，即使有再好的功能也会输在起跑线。** 虽然没必要过早优化性能，但性能一般是大多数项目选择库的第一考量。也是众多框架激烈取舍的重点。另外如需考虑弱网络环境下应用的初始加载速度，包和必需组件包的大小（如redux+react-redux）也会有一定影响。包大小也能间接反映内建复杂度，库做得越多 = 业务要做的越少
- **功能**：复杂的应用可能会有特别的功能需求，如有些需要时间回溯来撤销与重现，有些需要和其他库（路由等）联动使用；另外，部分库可能有不向前兼容的更新，也会影响功能的实现或增加更新的成本。
...
Rematch 相当于给redux提供了语法糖，大大简化了store、action、reducer的创建过程，采用原生async/await处理异步，同时又可以使用redux生态的各种中间件。详情可见[对比](https://rematch.gitbook.io/handbook/mu-de)
